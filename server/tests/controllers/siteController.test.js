...
describe('Site API', () => {
  ...
  describe('POST /api/sites', () => {
    it('returns 400 for invalid email', async () => {
      const res = await request(app).post('/api/sites').set(headers)
        .send({ url: 'https://example.com', email: 'bad-email', checkInterval: 1 });
      expect(res.status).toBe(400);
    });

    it('returns 400 for invalid checkInterval', async () => {
      const res = await request(app).post('/api/sites').set(headers)
        .send({ url: 'https://example.com', email: 'a@b.com', checkInterval: 3 });
      expect(res.status).toBe(400);
    });
    ...
    it('creates a site with active status when scrape succeeds', async () => {
      ...
      const res = await request(app).post('/api/sites').set(headers)
        .send({ url: 'https://example.com', email: 'a@b.com', checkInterval: 1 });
    ...
    it('returns 409 for duplicate URL on same account', async () => {
      ...
      await request(app).post('/api/sites').set(headers)
    ...
    it('returns 403 when monitor limit (20) is reached', async () => {
      ...
      for (let i = 0; i < 20; i++) {
        await request(app).post('/api/sites').set(headers)
          .send({ url: `https://site${i}.com`, email: 'a@b.com', checkInterval: 1 });
      }
      const res = await request(app).post('/api/sites').set(headers)
        .send({ url: 'https://site21.com', email: 'a@b.com', checkInterval: 1 });
    ...
    it('truncates label to 60 characters', async () => {
      ...
      const res = await request(app).post('/api/sites').set(headers)
        .send({ url: 'https://labeltest.com', email: 'a@b.com', checkInterval: 1, label: longLabel });
  ...
  describe('DELETE /api/sites/:id', () => {
    it('returns 404 if site not found or not owned by user', async () => {
      const res = await request(app).delete('/api/sites/000000000000000000000000').set(headers);
      expect(res.status).toBe(404);
    });

    it('deletes a site successfully', async () => {
      scrapeText.mockResolvedValue('content');
      const create = await request(app).post('/api/sites').set(headers)
        .send({ url: 'https://deleteme.com', email: 'a@b.com', checkInterval: 1 });
  ...
  describe('POST /api/sites/check-now/:id', () => {
    it('returns 404 if site not found', async () => {
      const res = await request(app).post('/api/sites/check-now/000000000000000000000000').set(headers);
      expect(res.status).toBe(404);
    });
    ...
    it('marks site unreachable if scrape fails on check-now', async () => {
      ...
      const create = await request(app).post('/api/sites').set(headers)
        .send({ url: 'https://checknow1.com', email: 'a@b.com', checkInterval: 1 });
      const id = create.body.data._id;

      scrapeText.mockRejectedValueOnce(new Error('down'));
      const res = await request(app).post(`/api/sites/check-now/${id}`).set(headers);
      expect(res.status).toBe(200);
    ...
    it('detects a change, sends alert, and updates hash', async () => {
      ...
      const create = await request(app).post('/api/sites').set(headers)
        .send({ url: 'https://checknow2.com', email: 'a@b.com', checkInterval: 1 });
      const id = create.body.data._id;
    ...
    it('reports no change and does not send an email', async () => {
      ...
      const create = await request(app).post('/api/sites').set(headers)
        .send({ url: 'https://checknow3.com', email: 'a@b.com', checkInterval: 1 });
      const id = create.body.data._id;
      const res = await request(app).post(`/api/sites/check-now/${id}`).set(headers);
      expect(res.status).toBe(200);
    ...
    it('caps changeHistory at 10 entries', async () => {
      ...
      const create = await request(app).post('/api/sites').set(headers)
        .send({ url: 'https://history.com', email: 'a@b.com', checkInterval: 1 });
      const id = create.body.data._id;
